...
describe('RFC9421', () => {
	...
	describe(signAsRFC9421ToRequestOrResponse, () => {
		test('basic', async () => {
			...
			expect(request.headers['Signature']).toBe(`sig1=:${basicExpectedSignature}:`);
		});
	});
			...
			expect(parsed.value).toStrictEqual([
				[
					'sig1',
					{
						...
						signature: basicExpectedSignature,
						tag: undefined,
					},
				],
			]);
